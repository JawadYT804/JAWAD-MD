  ...
  Jawad.ev.on('messages.upsert', async(mek) => {
    ...
    if (isGroup && !isAdmins && isBotAdmins) {
      ...
      if (urlRegex.test(cleanBody)) {
        ...
        let userWarnings = global.userWarnings[sender] || 0;
        ...
        if (config.ANTILINK === "true") {
          ...
          userWarnings += 1;
          global.userWarnings[sender] = userWarnings;
          
          if (userWarnings <= 3) {
            await Jawad.sendMessage(from, { delete: mek.key });
            await Jawad.sendMessage(from, {
              text: `⚠️ @${sender.split('@')[0]}, this is your ${userWarnings} warning. Please avoid sharing links.`,
              mentions: [sender]
            }, { quoted: mek });
          } else {
            await Jawad.sendMessage(from, { delete: mek.key });
